...
public class LoginManager {
    ...
    public ClientService login(String email, String password, ClientType clientType) throws UserErrorException {
        switch (clientType) {
            case ADMINISTRATOR:
                adminImpl.adminLogin(email, password);
                return adminImpl;

            case COMPANY:
                companiesImpl.companyLogin(email, password);
                int companyId = companiesImpl.getCompanyId(email, password);
                companiesImpl.setCompanyId(companyId);
                return companiesImpl;

            case CUSTOMER:
                customersImpl.customerLogin(email, password);
                int customerId = customersImpl.getCustomerId(email, password);
                customersImpl.setCustomerId(customerId);
